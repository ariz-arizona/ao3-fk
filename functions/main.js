...
const set = async () => {
  ...
  await bot.sendMessage(chatId, "Выберите битву:", {
    reply_markup: {
      inline_keyboard: [
        [
    ...
    .then(() => {
      if (otherLinks.length) {
        otherLinks.forEach((link) => {
          return bot.sendMessage(
            chatId,
            `Я нашел видео, посмотрите его по ссылке:\n${link}`
          );
        });
      }
      media.forEach((img) => {
        bot.sendMediaGroup(chatId, img).then(
          ...
          () => {
            bot.sendMessage(
              chatId,
              `Не смог отправить картинки, поэтому вот ссылки:\n${img
                .map((el) => el.media)
                .join("\n")}`
            );
          }
  ...
  domLinks.forEach((el) => {
    if (
      el
        .closest(".collection")
        .querySelector(".type")
        .innerText.indexOf("Unrevealed") === -1
    ) {
      const link = {};
      link.name = el
        .closest(".collection")
        .querySelector(".heading > a").textContent;
...
const workParserFinder = async (url) => {
  ...
  const pagesCount = dom.querySelector(".pagination li:nth-last-child(2)")
    ? dom.querySelector(".pagination li:nth-last-child(2)").textContent
    : 1;
...
const makeEmbed = (
  ...
  const url =
    randomWorkUrl.indexOf("http") === 0
      ? randomWorkUrl
      : `${ao3Url}${randomWorkUrl}`;
  const embed = {
    type: "rich",
    title: title,
    url: url,
  ...
  if (author && author.length)
    embed.footer = {
      icon_url:
        "https://s3.amazonaws.com/otw-ao3-icons/icons/7145533/standard.png",
      text: author.join(", "),
    };
